...
//   table read ok, mode absent         → the table's 'solo' row
//   table unreachable/slow/empty,
//   or 'solo' row somehow missing      → the hardcoded floor
// The returned mode is the one whose prompt actually answered — an
// unrecognised requested mode records as 'solo' on the row.
async function resolvePrompt(
  ...
  if (rows) {
    ...
    const solo = rows.get("solo");
    if (solo !== undefined) return { mode: "solo", version: solo, from: "table" };
  }
  return { mode: "solo", version: PROMPT_VERSION_FLOOR, from: "fallback" };
}
