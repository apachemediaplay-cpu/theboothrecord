...
### Per-venue prompts (Layer 1 only)  ·  `src/lib/source.ts`
Slug-keyed `VENUES` returns `{ displayName, headline, guidance? }`; unknown/missing source → `DEFAULT_PROMPT` (`"Confess something."`, no guidance). One source of truth per slug (display name **and** prompt). Tailored *input* → tailored *output*; the verdict engine is **not** given venue context (Layer 2 deliberately not built). Live slugs: `seoultiger1988`, `frenchiecbda`, `frenchiecbdb`, `frenchiecbd`.
...
| Venue-prompt config + attribution + session/test helpers | `src/lib/source.ts` |
